...
def _check_queue_filters(course: Course, query_params):
    ...
    for i, assignment in enumerate(assignments_queryset):
        is_selected = assignment.submission_type == AssignmentFormat.ONLINE
        assignments.append({
            "value": assignment.pk,
    ...
    # Course teachers
    course_teachers = [{"value": "unset", "label": "Unassigned", "selected": False}]
...
class AssignmentDetailView(PermissionRequiredMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["can_download_status_report"] = self.object.submission_type == AssignmentFormat.ONLINE
...
class AssignmentStatusLogCSVView(PermissionRequiredMixin, generic.DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if assignment.submission_type != AssignmentFormat.ONLINE:
        ...
        for comment in comments:
            ...
            if is_status_changed_on_completed:
                action_text = 'Grade updated'
            elif is_publish_online_solution:
                action_text = 'Solution submitted for review'
            elif is_status_changed_on_needfixes:
                action_text = 'New comment received from reviewer'
...
class StudentAssignmentDetailView(PermissionRequiredMixin,
    ...
    def form_invalid(self, form: AssignmentReviewForm):
        ...
        messages.error(self.request, "Unable to save data.<br>" + msg, extra_tags="timeout")
